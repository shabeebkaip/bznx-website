...
export async function POST(request: Request) {
  try {
    ...
    const existing = await About.findOne({ section: sectionName });

    if (!existing) {
      // Create new section
      const newSection = await About.create({ section: sectionName, content });
...
export async function DELETE(request: Request) {
  try {
    ...
    const section = await About.findOne({ section: sectionName });
    if (!section) {
