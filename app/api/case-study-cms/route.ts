...
export async function GET() {
  try {
    ...
    const allSections = await CaseStudyCMS.find();
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await CaseStudyCMS.findOne({ section: sectionName });

    if (!existing) {
      const newSection = await CaseStudyCMS.create({ section: sectionName, content });
    ...
    const updatedSection = await CaseStudyCMS.findOneAndUpdate(
      { section: sectionName },
...
export async function DELETE(request: Request) {
  try {
    ...
    const section = await CaseStudyCMS.findOne({ section: sectionName });
    if (!section) {
