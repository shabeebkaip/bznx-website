...
export async function GET() {
  try {
    await dbConnect();
    const allSections = await Footer.find();
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await Footer.findOne({ section: sectionName });

    if (!existing) {
      // Create new section
      const newSection = await Footer.create({ section: sectionName, content });
    ...
    const updatedSection = await Footer.findOneAndUpdate(
      { section: sectionName },
...
export async function DELETE(request: Request) {
  try {
    ...
    if (Array.isArray(existingContent)) {
      ...
      // If content is an object but no listKey, we treat it as an array if possible (fallback)
      if (Array.isArray(existingContent)) {
         updatedContent = existingContent.filter((item: any) => item._id?.toString() !== id);
      } else {
